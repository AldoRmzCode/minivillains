...
class GameScene extends Phaser.Scene {
	...
	preload() {    
		...
		this.loadtext= this.add.text(this.canvas.width/2, this.canvas.height/2, "Connecting...", {fontFamily: "Bubblegum Sans", fontSize: "32px", color: "#ffffff"}).setOrigin(0.5).setScrollFactor(0, 0).setDepth(200);
		this.ping = 0;
	...
	create() {
		...
		grecaptcha.ready(() =>{
			grecaptcha.execute(CAPTCHASITE, {action: "join"}).then((thetoken) => {
				...
				this.s2 = this.sound.add("s2", config);
				this.s3 = this.sound.add("s3", config);
				this.s4 = this.sound.add("s4", config);
				this.s5 = this.sound.add("s5", config);
				...
				//player+fpscounter
					...
					this.playerCount = this.add.text(0, 0, "Players: 0" + (!this.mobile ? "\nFPS: 0\nTPS: 0\nPing: 0 ms":""), {
						fontFamily: "Bubblegum Sans",
						align: "right"
					}).setFontSize(15).setDepth(101).setOrigin(1)/*.setStroke('#000000', 5)*/;
					this.playerCount.setScrollFactor(1);

					//leaderboard
					this.leaderboard = this.add.rexBBCodeText(0, 10, "", {
					fontFamily: "Bubblegum Sans",
				}).setFontSize(15).setDepth(101);
				...
				this.miniGraphics.lineStyle(5, 0x414141, 1); // borde minimapa
				...
				this.ability = this.add.text(this.abilityButton.x, this.abilityButton.y-100, "").setDepth(101).setVisible(false).setFontSize(50).setFontFamily("Bubblegum Sans").setOrigin(0.5)/*.setStroke('#000000', 5)*/;
		  ...
		  if(this.spectating) return;
				...
				//lvl text
				try {
				this.lvlText = this.add.text(this.canvas.width / 2, this.canvas.height / 5,  "", {fontFamily: "Bubblegum Sans", color: "#44DF0E"}).setFontSize(convert(1366, 35, this.canvas.width)).setDepth(75).setAlpha(0).setOrigin(0.5); //subiste de movel tamaño
				this.lvlText.setStroke('#000000', 5);
				this.lvlTextTween = undefined;

				this.lvlState = this.add.text(this.canvas.width / 2, this.lvlBar.y - (this.lvlBar.height),  "", { fontFamily: "Bubblegum Sans", color: "#DD5F17"}).setFontSize(convert(1366, 35, this.canvas.width)).setDepth(75).setAlpha(1).setOrigin(0.5);
				this.lvlState.setStroke('#000000', 5);
          ...
          if(i%2==0) return;
				...
				const resize = () => {
					...
					try {
						...
						this.miniGraphics.x = this.canvas.width - ((this.miniMap.scaleFactor * 2) + padding);
						this.miniGraphics.y = this.canvas.height - ((this.miniMap.scaleFactor * 2) + padding);
						this.miniGraphics.lineStyle(5, 0x000000, 1); // color afuera del juego
						this.miniGraphics.strokeRoundedRect(0, 0, this.miniMap.scaleFactor * 2,  this.miniMap.scaleFactor * 2, 0);
				...
				const addPlayer = (player) => {
					...
					var enemy = {
						...
						nameTag: this.add.rexBBCodeText(player.pos.x, player.pos.y - 90, `${player.name}`, {
							fontFamily: "Bubblegum Sans",
							fill: player.verified?player.name.toLowerCase()=="tiaret" ||player.name.toLowerCase()=="kazaf"||player.name.toLowerCase()=="Marseloselo"?"#FF0000":"#0000FF" :"#FFA616",
							fontSize: "25px"
          ...
          for (var remain of remaining) {
            ...
            this.miniMap.people.splice(this.miniMap.people.findIndex(x => x.id === remain.id), 1);
				...
				this.socket.on("me", (player) => {
					...
					this.lvlState.setText(/*"Nivel: " + player.level +" ("+Math.round((lvlcoins/diff)*100)+"%)"*/);
					...
					if(this.myObj && player.level > this.myObj.level) {
						...
						//this.lvlText.setText("Subiste de nivel!"+(this.lvlText.getData("x") > 1 ? ` x${this.lvlText.getData("x")}` : ""));
					...
					if((this.meSword.texture.key != (player.evolution == "" ? player.skin+"Player" : player.evolution+"Player"))) {
					...
					} else this.meSword.setTexture(player.evolution+"Player");
				...
				this.socket.on("player", (player) => {
					...
					try {
						...
						//skin + evo update
						if((enemy.sword.texture.key != (player.evolution == "" ? player.skin+"Player" : player.evolution+"Player"))) {
						...
						} else enemy.sword.setTexture(player.evolution+"Player");
          ...
          if(data.id == this.myObj.id)  {
			...
			enemy.chatTween = this.tweens.add({
				...
				if(this.myObj && data && this.myObj.id === data.killedBy.id) {
					...
					//i killed them!!
						var fontsize = convert(1366, 30, this.canvas.width);
						if(Date.now() - this.lastKill < 2500) {
							this.streak++;
							var txt = "";
							var list = ["DOBLE", "TRIPLE", "CUADRA", "QUINTA", "SEXTA", "SEPTIMA", "OCTA", "NONA", "DECA"];
					...
					//var msgs = ["EZ", "WOW","VICTORY","COOL","YESS","EASY","PRO","KING","SABOTAGE","EJECTED"];
	        //var msg = msgs[Math.floor(Math.random() * msgs.length)];
	        //this.randomText = msg;
					//var txt = `${msg}`;

						}
					var colormsgs = ["#FF8007","#cc0000"];
	        var colormsg = colormsgs[Math.floor(Math.random() * colormsgs.length)];
					var text = this.add.text(this.canvas.width/2, this.canvas.height/2, txt, {fontFamily: "Bubblegum Sans", color: colormsg}).setOrigin(0.5).setAlpha(0).setFontSize(fontsize);
				...
				var randomhit = ["s1","s2","s3","s4","s5"];
				var hitsound = randomhit[Math.floor(Math.random() * randomhit.length)];
				...
				var msg = msgs[Math.floor(Math.random() * msgs.length)];
					...
					//var msgs = ["epicCoin", "burgerCoin", "fresaCoin", "kekiCoin", "manzaninCoin", "burgersiCoin", "chocoCoin"];
						...
						var particles = this.add.particles(msg);
				...
				var randomhit = ["s1","s2","s3","s4","s5"];
				var hitsound = randomhit[Math.floor(Math.random() * randomhit.length)];
				this.damage = this.sound.add(hitsound, config);
				var msgs = ["hitPro","hitWow","hitEasy", "hitEjected", "hitEz", "hitGg", "hitLol", "hitNice", "hitOmg", "hitVictory", "hitXd", "hitYes"];
				//var msgs = ["epicCoin", "burgerCoin", "fresaCoin", "kekiCoin", "manzaninCoin", "burgersiCoin", "chocoCoin"];
				var msg = msgs[Math.floor(Math.random() * msgs.length)];
				...
				const addCoin = (coin,start) => {
					...
					this.coins.push(
						{
							id: coin.id,
							item: this.add.image(start[0], start[1], msg).setScale(coin.size/100).setDepth(20).setAlpha(anim?0:1),
		...
		*/
		...
		try {
			...
			sorted.slice(0,limit).forEach((entry, i) => {
				...
				text += `#${i+1}: ${playerObj.verified? playerObj.name.toLowerCase()=="tiaret" ||playerObj.name.toLowerCase()=="kazaf"||playerObj.name.toLowerCase()=="Marseloselo" ?"[color=#FF0000]":"[color=#0000FF]":""}${playerObj.name}${playerObj.verified? "[/color]":""}${rankingColor ? `[color=${rankingColor}](#${playerObj.ranking})[/color]` : ""}- ${conv(playerObj.coins)}\n`;
			...
			if(!amIinit) {
				...
				text += `...\n#${myIndex+1}: ${playerObj.verified? playerObj.name.toLowerCase()=="tiaret" ||playerObj.name.toLowerCase()=="kazaf"||playerObj.name.toLowerCase()=="Marseloselo" ?"[color=#FF0000]":"[color=#0000FF]":""}${playerObj.name}${playerObj.verified? "[/color]":""}${rankingColor ? `[color=${rankingColor}](#${playerObj.ranking})[/color]` : ""}- ${conv(playerObj.coins)}\n`;
