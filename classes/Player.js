 ...
 if(controller.up) {
  ...
  collectCoins(coins, io, levels) {
        ...
        touching.forEach((coin) => {
          ...
          var levelsPassed = [];
                ...
                var evoLevels = levelsPassed.slice(oldLevel-this.level).filter(level => level.evolutions)?.map((e)=>e.evolutions).map((e)=>e.map((f)=>f.name));
                this.evolution = [...this.evolutionQueue, ...evoLevels].filter((e)=>e)[0] ?? this.evolution;
  ...
  checkSubEvolutions() {
    if(
      ...
      ) this.evolution = (evolutions[this.evolution].subEvolutions.slice(1).map((e)=>e.name))[0];
      //this.updateValues();
