...
var throwAbi = 1; // desactiva el lanzamiento del click derecho
...
var oldlevels = [
	{coins: 10, scale: 0.38, evolutions: [evolutions.e1]},
  {coins: 20, scale: 0.39, evolutions: [evolutions.e2]},
  {coins: 35, scale: 0.40, evolutions: [evolutions.e3]},
  {coins: 55, scale: 0.41, evolutions: [evolutions.e4]},
  {coins: 80, scale: 0.42, evolutions: [evolutions.e5]},
  {coins: 110, scale: 0.43, evolutions: [evolutions.e6]},
...
app.all("*", (req, res, next) => {
  ...
  if (moderation.bannedIps.includes(ip)) {
    res.status(403).send("Tu has sido baneado, contactar con  soporte@damequetedoy.com para solucionar el problema<br>Que tenga buen dia :)");
    return;
  } else next();
} catch (e) {
  // console.log(e);
...
app.get("/settings", async (req, res) => {
  res.send(
    "I'm still working on this page.<br><br>For now, if you want to change password, or change your username, please email<br>support@damequetedoy.com"
  );
});
...
io.on("connection", async (socket) => {
  ...
  if (moderation.bannedIps.includes(socket.ip)) {
    socket.send(
      "ban",
      "You are banned. Appeal to appeals@damequetedoy.com<br><br>BANNED IP: " +
        socket.ip
    );
    socket.disconnect();
  }
    ...
    async function ready() {
      ...
      if (options && options.hasOwnProperty("movementMode")) {
		...
		if (!captchatoken && recaptcha) {
			socket.send(
				"ban",
				"You were kicked for not sending a captchatoken. Send this message to bugs@damequetedoy.com if you think this is a bug."
			);
		...
		if (PlayerList.has(socket.id)) {
			socket.send(
				"ban",
				"You were kicked for 2 players on 1 id. Send this message to support@damequetedoy.com<br> In the meantime, try restarting your computer if this happens a lot. "
			);
			return socket.disconnect();
		}
          ...
          new URLSearchParams(send)
				...
				.then((f) => {
					...
					if (f.score < 0.3) {
						socket.send(
							"ban",
							`Captcha score too low: ${f.score}<br><br>If you're using a vpn, disable it. <br>If your on incognito, go onto a normal window<br>If your not signed in to a google account, sign in<br><br>If none of these worked, contact support@damequetedoy.com`
						);
						socket.disconnect();
						return;
					}
  ...
  socket.on("ability", () => {
    ...
    if(player.evolution != "") {
      ...
      if(player.ability <= Date.now()) {
        ...
        console.log(player.name + " activated ability " + player.evolution);
        socket.send("ability", [evolutions[player.evolution].abilityCooldown , evolutions[player.evolution].abilityDuration, Date.now()]);
      }
      if(player && player.evolution && player.evolution.length > 0 && player.evolution[0].includes(player.evolution)) {
        eclass = player.evolution;
        player.evolutionQueue.shift();
        var evo = evolutions[eclass];
        console.log(player.name + " evolved to " + eclass);
...
console.log(thePlayer.name + " - " + socket.id + " disconnected");
              //drop their coins
              var drop = [];
              var dropAmount = thePlayer.coins < 3 ? 10 : Math.round(thePlayer.coins < 25000 ? thePlayer.coins * 0.8 : Math.log10(thePlayer.coins) * 30000 - 111938.2002602);
              var dropped = 0;
